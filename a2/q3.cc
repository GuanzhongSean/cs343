...
_Task increment{void main(){for (decltype(iterations) i = 0; i < iterations; i += 1){
...
int main(int argc, char* argv[]) {
	...
	try {  // process command-line arguments
		switch (argc) {
			case 3:
				processors = convert(argv[2]);
				if (processors <= 0) throw cmd_error{};
			case 2:
				iterations = convert(argv[1]);
				if (iterations <= 0) throw cmd_error{};
			case 1:
				break;	// use defaults
			default:
				throw cmd_error{};
